...
#Ask the user to choose a Klingon consonant they want to practice with. Ask again if the user’s answer is not a valid Klingon consonant. Keep asking until the user enters a valid consonant.
...
#If the answer is incorrect in the first attempt, show a hint: the first and last characters of the correct Klingon word. When showing a hint, replace all other characters with a star (*)
#If the answer is still incorrect in the second attempt, show the same hint once again.
#You will refactor in this version:
#If the answer is incorrect in the first attempt, show the first hint: the first and last characters of the correct Klingon word. When showing a hint, replace all other characters with a star (*).  This feature is already implemented but you may want to refactor the code because of the addition of the second hint
#You will add in this version:
#If the answer is incorrect in the second attempt, show the second hint: the first and the last characters plus an extra random character of the correct Klingon word. Please note that you will need to use the random module to complete this task. Therefore, import the random module at the start of your program.
#Already implemented:
            ...
            if self.translation in n[0] or self.translation in (n[0] + n[1]):
                ...
                if tries == 2: #Hint sequence
                   ...
                   if inp2 == split[0]: #General if else statement for try 2
                       print("correct")
                   if inp2 != split[0]:
                       tranattemp2 = [] #Beginning of logic for randomized hint
                       for n in range(len(tran)):
                           tranattemp2.append(n) #Appends indicies of word to an array
                       tranattemp2.remove(0) #Removes the first indicy
                       tranattemp2.remove(max(tranattemp2)) #Removes last indicy
                       randnum = random.randint(1,(len(tranattemp2))) #Randomizes letter with a range from the second letter to the second last
                       if randnum > 1: #If the random number is not the second letter
                           print("your hint is",tran[0],"*"*(randnum-1),tran[randnum],"*"*((len(tran)-2)-randnum),tran[len(tran)-1]) #Math
                       if randnum <= 1: #If the random number is the second letter
                           print("your hint is",tran[0],tran[1],"*"*(len(tran)-3),tran[len(tran)-1]) #Math
                       inp2 = input("what is your new guess?\n") #General if else statement for try 3                          
                       if inp2 == split[0]:
                           print("correct")
                       if inp2 != split[0]:
